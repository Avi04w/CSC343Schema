-- Assuming that the average passengers requiring visas between country A 
and country B is calculated by # people requiring visas / # of flights
DROP VIEW IF EXISTS PairAverages CASCADE;
DROP VIEW IF EXISTS GlobalAverage CASCADE;


CREATE VIEW PairAverages AS (
	SELECT a1.country AS c1, a2.country AS c2, 
	COUNT(p.pass_id) / COUNT(DISTINCT sf.f_id) AS avg
